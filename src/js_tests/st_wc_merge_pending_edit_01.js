...
function st_wc_merge_edit_01()
{
    var my_group = "st_wc_merge_edit_01";	// this variable must match the above group name.
	...
	// re-edit the file and overwrite what B did.
	// auto-merge should still succeed, but it will
	// contain *ONLY* reflect (A,WC,C) and not B.

	vscript_test_wc__write_file("file_A.txt", (content_W1 + content_equal));
	...
	// we expect auto-merge to produce this result.
	var mrg = (content_W1 + content_equal + content_C1);
	var f = sg.file.read("file_A.txt");
	testlib.ok( (f == mrg),
		    "Content of file_A.txt." );
