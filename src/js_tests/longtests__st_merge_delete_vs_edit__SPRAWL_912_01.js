...
function st_merge_delete_vs_edit__SPRAWL_912_01()
{
	...
	// With W4887, we don't get a conflict-name unless we really need it.
	// The original purpose of this test was to manually rename the conflict
	// name back to the original and get the implicit resolve that was
	// added in W5226.  So do a little shuffle to force it thru that code.

	vscript_test_wc__rename( "dir_1/file_11", "abcd_11" );
	vscript_test_wc__rename( "dir_1/abcd_11", "file_11" );

	// since it was added, the rename is silently converted
	// into an add with the new name.
	//
	// See W5226 where we want the RENAME to implicitly resolve (part of) the conflict.
	...
	print(o.stdout);
        ...
        expect_test["Added (Merge)"] = [ "@/dir_3",
					 "@/dir_1/file_11" ];
	expect_test["Resolved"] = [ "@/dir_1/file_11" ];	// see W5226.
	expect_test["Choice Resolved (Existence)"] = [ "@/dir_1/file_11" ];	// see W5226.
        vscript_test_wc__confirm_wii(expect_test);
	...
	var result = sg.exec(vv, "diff");
	print(dumpObj(result, "sg.exec()", "", 0));
	testlib.ok( (result.exit_status == 0), "DIFF should pass");

	// try to REVERT the file.  See question in _pt_revert__revert_cancel_added()
	// about what this should do.  for now, i'm going to assume that the revert
	// will revert BOTH the 'user-delete' *and* the 'add-by-merge'.  so the file
	// should be gone and the status clean WRT the baseline.

	var result = sg.exec(vv, "revert", "--all");
	print(dumpObj(result, "sg.exec()", "", 0));
	testlib.ok( (result.exit_status == 0), "REVERT should pass");
