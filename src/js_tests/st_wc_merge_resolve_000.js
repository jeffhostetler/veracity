...
function st_wc_merge_resolve_000()
{
    var my_group = "st_wc_merge_resolve_000";	// this variable must match the above group name.
	...
	this.gid_BB = sg.wc.get_item_gid_path({"src":"dir_X/dir_BB"});
	...
	expect_test["Added (Merge)"]                 = [ this.gid_CX ];
	...
	var expect_test = new Array;
	expect_test["Added (B)"]                     = [ this.gid_BX, this.gid_BB ];
	expect_test["Added (C)"]                     = [ this.gid_CX ];
	...
	expect_test["Resolved"]                      = [ this.gid_BX ];
	expect_test["Choice Resolved (Existence)"]   = [ this.gid_BX ];
	...
	expect_test["Added (C)"]                   = [ "@c/dir_X" ];
	expect_test["Removed (WC)"]                = [ "@c/dir_X" ];
