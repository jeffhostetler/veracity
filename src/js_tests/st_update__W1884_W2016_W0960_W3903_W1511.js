...
function st_update__W1884_W2016_W0960_W3903_W1511()
{
    this.my_group = "st_update__W1884_W2016_W0960_W3903_W1511";	// this variable must match the above group name.
	...
	//////////////////////////////////////////////////////////////////
	vscript_test_wc__print_section_divider("Create alternate merge results: 'merge'");

	vscript_test_wc__resolve__merge(":baseline", "@/file_A.txt");
	vscript_test_wc__resolve__accept("merge", "@/file_A.txt", "--overwrite");
	...
	// Cause:       Edit/Edit
	//              Changes to item's contents in different branches conflict.
	// Status:      Resolved (accepted merge)
	// Choices:     Accept a value from below to change this conflict's resolution.
	//    ancestor: File's contents from the merge's common ancestor changeset
	//    baseline: File's contents from the original parent changeset
	//       other: File's contents from the merged-in changeset
	//   automerge: Merge of baseline and other using :merge from Jun 13 at 12:13
	//       merge: Merge of baseline and other using :baseline from Jun 13 at 12:13
	//     working: File's current contents in the working copy (from merge)

	var auto_merged_content = sg.file.read("file_A.txt");
	testlib.ok( (auto_merged_content == X1_dirt1_content), "Auto-Merge on file_A.txt.");

	//////////////////////////////////////////////////////////////////
	vscript_test_wc__print_section_divider("Create alternate merge results: 'merge2'");

	vscript_test_wc__resolve__merge(":other", "@/file_A.txt");
	vscript_test_wc__resolve__accept("merge2", "@/file_A.txt", "--overwrite");
	...
	// Cause:       Edit/Edit
	//              Changes to item's contents in different branches conflict.
	// Status:      Resolved (accepted merge2)
	// Choices:     Accept a value from below to change this conflict's resolution.
	//    ancestor: File's contents from the merge's common ancestor changeset
	//    baseline: File's contents from the original parent changeset
	//       other: File's contents from the merged-in changeset
	//   automerge: Merge of baseline and other using :merge from Jun 13 at 12:13
	//       merge: Merge of baseline and other using :baseline from Jun 13 at 12:13
	//      merge2: Merge of baseline and other using :other from Jun 13 at 12:13
	//     working: File's current contents in the working copy (from merge2)

	var auto_merged_content = sg.file.read("file_A.txt");
	testlib.ok( (auto_merged_content == X3_content), "Auto-Merge on file_A.txt.");

	//////////////////////////////////////////////////////////////////
	vscript_test_wc__print_section_divider("Remove the resolved file and see what that does to RESOLVE.");

	vscript_test_wc__remove("@/file_A.txt");

	var expect_test = new Array;
	expect_test["Removed"]              = [ "@b/file_A.txt" ];
	expect_test["Resolved"]             = [ "@b/file_A.txt" ];
	expect_test["Choice Resolved (Contents)"] = [ "@b/file_A.txt" ];
	vscript_test_wc__confirm_wii(expect_test);

	// W0960.  We should have preserved the custom 'merge' and 'merge2' choices.
	...
	//    baseline: File's contents from the original parent changeset
	//       other: File's contents from the merged-in changeset
	//   automerge: Merge of baseline and other using :merge from Jun 13 at 14:16
	//       merge: Merge of baseline and other using :baseline from Jun 13 at 14:16
	//      merge2: Merge of baseline and other using :other from Jun 13 at 14:16

	//////////////////////////////////////////////////////////////////
	vscript_test_wc__print_section_divider("Try to restore (un-delete) it back using a cset-based version (with mnemonic in value).");

	vscript_test_wc__resolve__accept("other", "@b/file_A.txt", "--overwrite");
	...
	//////////////////////////////////////////////////////////////////
	vscript_test_wc__print_section_divider("Remove the resolved file again.");
	...
	//////////////////////////////////////////////////////////////////
	vscript_test_wc__print_section_divider("Try to restore (un-delete) it back using a custom (non-cset) version.");

	vscript_test_wc__resolve__accept("merge", "@b/file_A.txt", "--overwrite");
