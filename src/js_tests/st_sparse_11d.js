...
function st_sparse_11d()
{
    this.my_group = "st_sparse_11d";	// this variable must match the above group name.
	...
	if (bWindows)
	    expect_test["Sparse"] = [ "@/sparse_link_1" ];
        vscript_test_wc__confirm(expect_test,
				 sg.wc.status( { "list_sparse" : true } ) );

	print("");
	print("Try to UPDATE with sparse files...");
	print("");

	var result = sg.wc.update( { "tag" : local_tag_L1,
				     "attach" : "master",
				    "verbose" : true } );
	...
	if (bWindows)
	{
        ...
        var expect_test = new Array;
	if (bWindows)
	    expect_test["Sparse"] = [ "@/sparse_link_1", "@/sparse_link_x" ];
        vscript_test_wc__confirm(expect_test,
