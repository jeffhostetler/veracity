...
function st_wc_merge_resolve_021__W1445()
{
    var my_group = "st_wc_merge_resolve_021__W1445";	// this variable must match the above group name.
	...
	vscript_test_wc__resolve__accept("baseline",    "@b/dirA2/portability.txt", "--overwrite");
	vscript_test_wc__resolve__accept("baseline",    "@c/dirA2/PORTABILITY.TXT", "--overwrite");
	...
	var expect_test = new Array;
	expect_test["Renamed (WC==A,WC==B,WC!=C)"] = [                            "@/dirA2/fileA2.txt" ];
	expect_test["Moved (WC!=A,WC==B,WC!=C)"]   = [ "@/dirA2/portability.txt"                       ];
	expect_test["Resolved"]                    = [ "@/dirA2/portability.txt", "@/dirA2/fileA2.txt" ];
	expect_test["Choice Resolved (Location)"]  = [ "@/dirA2/portability.txt"                       ];
	expect_test["Choice Resolved (Name)"]      = [                            "@/dirA2/fileA2.txt" ];
	vscript_test_wc__mstatus_confirm_wii(expect_test);

	// The point of W1445 is to ensure that a plain STATUS returns
	// resolve info for (now) unchanged items.

	vscript_test_wc__print_section_divider("RESOLVE 1: STATUS after accepts:");
