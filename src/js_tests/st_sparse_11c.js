...
function st_sparse_11c()
{
    this.my_group = "st_sparse_11c";	// this variable must match the above group name.
	...
	sg.wc.checkout( { "repo" : local_repoName,
			  "path" : ".",
			  "tag"  : local_tag_L0,
			  "attach" : "master" } );	// DO NOT include "sparse" on checkout.
	whoami_testing(local_repoName);
	...
	if (bWindows)
	    sparse_list.push( "@/sparse_link_x" );

	var x = sg.wc.status({"src":"@/sparse_link_x",
			      "list_sparse":true});
	testlib.ok( (x[0].status.isSymlink), "Is link.");
	testlib.ok( (x[0].WC.target == "/symlink/target/is/file_x"), "Target value.");
	if (bWindows)
	    testlib.ok( (x[0].status.isSparse), "Is sparse.");
	...
	// make sure that COMMIT doesn't barf on the sparse symlinks.

	var verbose = vscript_test_wc__commit_np( { "message" : "merge",
						    "test" : true,
						    "verbose" : true } );
	print(sg.to_json__pretty_print(verbose));

	var cset_merge = vscript_test_wc__commit_np( { "message" : "merge" } );
	sg.vv2.add_tag( { "rev" : cset_merge, "value" : "cset_merge" } );
	...
	if (bHavePosixSymlinks)
	{
	    ...
	    this.do_make_symlink("/symlink/target/is/file_1", "sparse_link_1");
	    ...
	    this.do_make_symlink("/symlink/target/is/file_x", "sparse_link_x");
	    vscript_test_wc__addremove();
