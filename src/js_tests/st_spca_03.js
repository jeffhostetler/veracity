...
function st_spca_01()
{
	...
	for (var k=1; k<kLimit; k++)
	{
	    ...
	    if (m1 == s1)
		this.merge_unique_with_letter__and_rename(p1, m1, f, f_r);	// merge(p0,p1) ==> m1
	    else
	...
	var p1a = p1 + "_a";
	this.do_fsobj_append_file(f_r, "Edited in " + p1a);
	...
	var p0a = p0 + "_a";
	this.do_fsobj_append_file(f_r, "Edited in " + p0a);
	...
	expect_test["Modified"]         = [ "@/" + f_r  ];
	expect_test["Auto-Merged"]      = [ "@/" + f_r  ];
	expect_test["Unresolved"]       = [ "@/" + f_r  ];
	expect_test["Choice Unresolved (Contents)"] = [ "@/" + f_r  ];
	...
	// 
	// since s0 edited the file and s1 renamed the file, we'll
	// get another set of
	...
	var issue = sg.wc.get_item_issues({"src": "@/" + f_r});
