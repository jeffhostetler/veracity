...
function st_wc_resolve__W1420() 
{
    this.my_group = "st_wc_resolve__W1420";	// this variable must match the above group name.
	...
	//
	// W8402: If the SECOND UPDATE will need to make *any* change to an ITEM
	...
	//////////////////////////////////////////////////////////////////
	// W1420 complains that we list "(non-existent)" for the current 
	// repo-path for the deleted items rather than using an extended-prefix
	// repo-path like the new 'vv status' does.
	...
	//////////////////////////////////////////////////////////////////
	vscript_test_wc__print_section_divider("RESOLVE ON DELETED ITEM");

	vscript_test_wc__resolve__check_if_resolved( "@b/b1/a1", true);

	var r = sg.exec(vv, "resolve", "list", "--all", "--verbose", "@b/b1/a1");

	//print(r.stdout);
	//print(r.stderr);

	var reItem = new RegExp("Item:[ \t]*(.*)");
	testlib.ok( (reItem.test(r.stdout)), "Found item line in output.");
	if (reItem.test(r.stdout))
	{
	    var values = r.stdout.match(reItem);
	    print("Match[0] : '" + values[0] + "'");
	    print("Match[1] : '" + values[1] + "'");
	    testlib.ok( (values[1] == "(@b/b1/a1/)"), "Found match" );
	}
