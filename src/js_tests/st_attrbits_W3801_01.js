...
function st_attrbits_W3801_01()
{
    this.my_group = "st_attrbits_W3801_01";	// this variable must match the above group name.
...
//	print(sg.to_json__pretty_print(s));
	...
	for (var j in result.journal)
	{
	    ...
	    {
		testlib.ok( (result.journal[j].op == "set_attrbits"), "Expect 'set_attrbits' for file_xoff");
		testlib.ok( (result.journal[j].attrbits == 1), "Expect 'attrbits=1' for file_xoff");
	    }
	    else if (result.journal[j].src == "@/file_xon")
	    {
		testlib.ok( (result.journal[j].op == "set_attrbits"), "Expect 'set_attrbits' for file_xon");
