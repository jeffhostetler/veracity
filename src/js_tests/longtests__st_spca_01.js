...
function st_spca_01()
{
	...
	// 
	// since neither s0 nor s1 changed the file, both parents
	// are equivalent and an equally good choice for the
	// ancestor for this file.
	//
	...
	testlib.ok( ((issue.input.A.cset_hid == this.letter_map[s0]) 
		     || (issue.input.A.cset_hid == this.letter_map[s1])),
		    "Expect issue.A to be either s0 or s1.");
    }
}
