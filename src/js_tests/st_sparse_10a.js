...
function st_sparse_10()
{
	...
	if (bChmod)
	{
	    this.do_fsobj_create_file("chmod_3");
	    this.do_fsobj_create_file("sparse_chmod_3");
	}
	if (bSymlinks)
	{
	    this.do_make_symlink("file_1", "sparse_link_1");
	    this.do_make_symlink("file_2", "sparse_link_2");
	}
	...
	this.do_fsobj_append_file("sparse_1", "more text");
	if (bChmod)
	{
	    this.do_chmod("chmod_3", 0700);
	    this.do_chmod("sparse_chmod_3", 0700);
	}
	if (bSymlinks)
	...
	this.do_fsobj_append_file("file_3", "more text");
	this.do_fsobj_append_file("sparse_2", "more text");
	if (bSymlinks)
	    this.do_make_symlink("file_3", "sparse_link_2");	// retarget it
	vscript_test_wc__addremove();
	var cset_L1 = vscript_test_wc__commit_np( { "message" : "L1 changes" } );

	print("cset_L1 created...");
	print(sg.to_json__pretty_print(sg.wc.status({"list_sparse":true,
						     "list_unchanged":true,
						     "no_ignores":true})));
	...
	if (bChmod)
	    sparse_list.push( "@/sparse_chmod_3" );
	if (bSymlinks)
	{
	    sparse_list.push( "@/sparse_link_1" );
	    sparse_list.push( "@/sparse_link_2" );
	}

	//////////////////////////////////////////////////////////////////
	vscript_test_wc__print_section_divider("WD 2 -- sparse checkout of L0");
	...
	//////////////////////////////////////////////////////////////////
	vscript_test_wc__print_section_divider("Try to MERGE in L1 with sparse files...");
	...
	expect_test["Modified"] = [ "@/file_3", "@/sparse_2"];
	if (bSymlinks)
	    expect_test["Modified"].push( "@/sparse_link_2" );
        vscript_test_wc__confirm(expect_test,
				 sg.wc.status( { "no_ignores" : true, 
						 "list_sparse" : true } ) );

	//////////////////////////////////////////////////////////////////
	vscript_test_wc__print_section_divider("WD 3 -- try again the other way -- L1 sparse checkout");
	...
	//////////////////////////////////////////////////////////////////
	vscript_test_wc__print_section_divider("Try to MERGE in L0 with sparse files...");

	var result = sg.exec(vv, "merge", "--rev", cset_L0, "--verbose");
	print(dumpObj(result, "sg.exec()", "", 0));
	testlib.ok( (result.exit_status == 0), "'vv update' should work" );
	...
	print(sg.to_json__pretty_print(sg.wc.status({"list_sparse":true,
        ...
        var expect_test = new Array;
	expect_test["Sparse"] = sparse_list;
	expect_test["Added (Merge)"] = [ "@/file_4", "@/file_5" ];
	expect_test["Modified"] = [ "@/sparse_1"];
	if (bSymlinks)
	    expect_test["Modified"].push( "@/sparse_link_1" );
	if (bChmod)
	    expect_test["Attributes"] = [ "@/chmod_3", "@/sparse_chmod_3" ];
        vscript_test_wc__confirm(expect_test,
				 sg.wc.status( { "no_ignores" : true, 
						 "list_sparse" : true } ) );
