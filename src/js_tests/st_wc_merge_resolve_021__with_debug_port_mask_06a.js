...
function st_wc_merge_resolve_021()
{
	...
	// Test that RENAME will IMPLICITLY resolve the items.
	...
	// Test that MOVE will IMPLICITLY resolve the items.

	vscript_test_wc__print_section_divider("RESOLVE 1: Preparing to MOVE files:");
	vscript_test_wc__move("@/dir/B__portability.txt", "@/");
	vscript_test_wc__move("@/dir/C__portability.txt", "@/");

	vscript_test_wc__print_section_divider("RESOLVE 1: MSTATUS after moves:");
	var o = sg.exec(vv,"mstatus", "--verbose");
	print(o.stdout);

	var expect_test = new Array;
	expect_test["Renamed (WC!=A,WC!=B,WC!=C)"] = [ "@/B__portability.txt", "@/C__portability.txt" ];
	expect_test["Moved (WC==A,WC!=B,WC==C)"]   = [ "@/B__portability.txt"                         ];
	expect_test["Moved (WC==A,WC==B,WC!=C)"]   = [                         "@/C__portability.txt" ];
	expect_test["Resolved"]                    = [ "@/B__portability.txt", "@/C__portability.txt" ];
	expect_test["Choice Resolved (Name)"]      = [ "@/B__portability.txt", "@/C__portability.txt" ];
	expect_test["Choice Resolved (Location)"]  = [ "@/B__portability.txt", "@/C__portability.txt" ];
	vscript_test_wc__mstatus_confirm_wii(expect_test);
