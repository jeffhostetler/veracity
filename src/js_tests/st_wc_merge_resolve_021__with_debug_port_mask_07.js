...
function st_wc_merge_resolve_021()
{
	...
	vscript_test_wc__write_file("dirA2/PORTABILITY.TXT", content_equal);
	...
	vscript_test_wc__move("dirA1/portability.txt", "dirA3");
	...
	vscript_test_wc__move("dirA2/PORTABILITY.TXT", "dirA3");
	...
	var B_pathname = "@/dirA3/portability.txt~g[0-9a-f]*";
	var C_pathname = "@/dirA3/PORTABILITY.TXT~g[0-9a-f]*";
	...
	vscript_test_wc__rename("@b/dirA3/portability.txt",  "B__portability.txt");
	vscript_test_wc__rename("@c/dirA3/PORTABILITY.TXT",  "C__portability.txt");

	vscript_test_wc__resolve__accept("working",    "@b/dirA3/portability.txt");
	vscript_test_wc__resolve__accept("working",    "@c/dirA3/PORTABILITY.TXT");

	var o = sg.exec(vv,"mstatus", "--verbose");
	print(o.stdout);

	var expect_test = new Array;
	expect_test["Renamed (WC!=A,WC!=B,WC!=C)"] = [ "@/dirA3/B__portability.txt", "@/dirA3/C__portability.txt" ];
	expect_test["Moved (WC!=A,WC==B,WC!=C)"]   = [ "@/dirA3/B__portability.txt"                               ];
	expect_test["Moved (WC!=A,WC!=B,WC==C)"]   = [                               "@/dirA3/C__portability.txt" ];
	expect_test["Resolved"]                    = [ "@/dirA3/B__portability.txt", "@/dirA3/C__portability.txt" ];
	expect_test["Choice Resolved (Location)"]  = [ "@/dirA3/B__portability.txt", "@/dirA3/C__portability.txt" ];
	vscript_test_wc__mstatus_confirm_wii(expect_test);
	...
	var o = sg.exec(vv, "resolve", "list", "--all", "--verbose", "@/dirA3/B__portability.txt");
	print(o.stdout);
	// Item:        @/dirA3/B__portability.txt
	// Conflict:    Location
	//              You must choose a location/parent for the item.
	// Cause:       Potential Collision
	//              Item's name matches a different item's name in another branch.
	// Related:     @/dirA3/C__portability.txt
	...
	var re1 = new RegExp("Cause:[ \t]*Potential Collision");
	var re2 = new RegExp("Related:[ \t]*" + "@/dirA3/C__portability.txt");
	testlib.ok( re1.test( o.stdout ), "1: Expect cause in verbose output.");
	testlib.ok( re2.test( o.stdout ), "2: Expect potential collision with other file." );
    }
