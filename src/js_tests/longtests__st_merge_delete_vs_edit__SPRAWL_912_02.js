...
function st_merge_delete_vs_edit__SPRAWL_912_02()
{
	...
	// since we already deleted the file, tell RESOLVE to mark it ok as is.

	var result = sg.exec(vv, "resolve", "accept", "working", "--all");
	print(dumpObj(result, "sg.exec()", "", 0));
	testlib.ok( (result.exit_status == 0), "Resolve should pass");

	// try to COMMIT everything.

	var result = sg.exec(vv, "commit", "-m", "Commit After Delete After Merge");
	print(dumpObj(result, "sg.exec()", "", 0));
	testlib.ok( (result.exit_status == 0), "Commit should pass");
