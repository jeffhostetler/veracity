...
function st_wc_merge_lost_01()
{
    var my_group = "st_wc_merge_lost_01";	// this variable must match the above group name.
	...
	sg.fs.remove("file_A.txt");

	var expect_test = new Array;
	expect_test["Lost"] = [ "@/file_A.txt"
			       ];
        vscript_test_wc__confirm_wii(expect_test);

	// With the changes for W3175 we don't allow dirty
	// merges by default.  Confirm that we get the dirty
	// merge error.
	vscript_test_wc__merge_np__expect_error( { "rev" : csets.C },
						 "This type of merge requires a clean working copy" );
	// But even if we allow a basic dirty merge, we
	// still need to fail because a lost item is just
	// too much to handle.
	vscript_test_wc__merge_np__expect_error( { "rev" : csets.C, "allow_dirty" : true },
						 "Operation not allowed with a lost item" );
