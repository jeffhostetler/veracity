...
function st_wc_merge_found_03()
{
    var my_group = "st_wc_merge_found_03";	// this variable must match the above group name.
	...
	vscript_test_wc__write_file("file_C.txt", content_equal);
	vscript_test_wc__addremove();
	...
	var expect_test = new Array;
	expect_test["Found"] = [ "@/FILE_C.TXT"
			       ];
        vscript_test_wc__confirm_wii(expect_test);


	vscript_test_wc__merge_np__expect_error( { "rev" : csets.C },
						 "The item 'FILE_C.TXT' would cause a portability conflict" );
