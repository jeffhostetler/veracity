...
function st_update__W1884_01__W7720_02() 
{
    this.my_group = "st_update__W1884_01__W7720_02";	// this variable must match the above group name.
	...
	vscript_test_wc__move("d1", "c1");		// now have @/c1/d1
	vscript_test_wc__remove("d2");
        this.my_commit("X2");
	...
	//////////////////////////////////////////////////////////////////
	// backup to X1 and create some dirt/conflicts
	...
	vscript_test_wc__remove( "d1" );
	vscript_test_wc__move("d2", "c1");

	//////////////////////////////////////////////////////////////////
	// update to X2 and get conflicts/issues on d1 and d2

	this.my_update("X2");

	var expect_test = new Array;
	expect_test["Added (Update)"] = [ "@/c1/d2" ];
	expect_test["Unresolved"]     = [ "@/c1/d1",
					  "@/c1/d2" ];
	expect_test["Choice Unresolved (Existence)"]     = [ "@/c1/d1",
							     "@/c1/d2" ];
	vscript_test_wc__confirm_wii(expect_test);
	...
	//////////////////////////////////////////////////////////////////
	// resolve the update issues so that the items are deleted (but the issues are still present).
	// For now we need to use a gid-domain path for d2 -- See W7521.

	this.d2 = this.my_find_unique_path("@/c1/d2~*");
	this.d2gid = '@' + sg.wc.status({"src":this.d2})[0].gid;

	vscript_test_wc__resolve__accept("baseline", "@b/c1/d1");	// use the existence status of the dirt
	vscript_test_wc__resolve__accept("other",    this.d2);		// use the existence status of X2

	var expect_test = new Array;
	expect_test["Added (Update)"] = [ this.d2gid ];
	expect_test["Removed"]        = [ "@b/c1/d1",
					  this.d2gid ];
	expect_test["Resolved"]       = [ "@b/c1/d1",
					  this.d2gid ];
	expect_test["Choice Resolved (Existence)"]       = [ "@b/c1/d1",
							     this.d2gid ];
	vscript_test_wc__confirm_wii(expect_test);
	...
	// Merge Y1 into X2+dirt.
	// For this test, we know that there are no conflicts between X2+dirt and Y1.
	// We just want to see how MERGE handles and reports on existing issues in
	// the WD for items that are only defined in the X1/X2 branch.  The LCA of this
	// merge is X0.
	//
	// With the changes for W2065 we should silently retire resolved issues
	// before we start the actual merge.
