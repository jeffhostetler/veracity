...
//
// The test here is for bug SPRAWL-rename.
//////////////////////////////////////////////////////////////////

function st_UpdateTestsComboIssues_rename_symlink() 
{
	...
	this.do_fsobj_create_file( "file_1", "This is the first version.");
	this.do_fsobj_create_file( "file_2", "This is the first version.");
	if ((sg.platform() == "LINUX") || (sg.platform() == "MAC"))
	{
	    this.do_make_symlink("file_1", "pointer_to_file_1");	// pointer_to_file_1 -> file_1
	    this.do_make_symlink("file_2", "pointer_to_file_2");	// pointer_to_file_1 -> file_1
	    this.do_make_symlink("bogus",  "pointer_to_bogus");		// pointer_to_bogus  -> bogus
	}
	vscript_test_wc__addremove();
	this.do_commit("Combo Trunk 1.1");

	this.do_fsobj_append_file("file_1", "new line for the second version; need this for windows build so that we have dirt to commit.");
	if ((sg.platform() == "LINUX") || (sg.platform() == "MAC"))
	{
	    vscript_test_wc__rename("pointer_to_file_1", "newNameFor_pointer_to_file_1");	// rename the pointer 
	    this.do_make_symlink("new_pointee",    "newNameFor_pointer_to_file_1");	// change the pointee

	    vscript_test_wc__rename("pointer_to_file_2", "newNameFor_pointer_to_file_2");	// rename the pointer

	    this.do_make_symlink("really_bogus", "pointer_to_bogus");	// change the pointee
	}
