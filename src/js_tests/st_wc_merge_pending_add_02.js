...
function st_wc_merge_pending_add_01()
{
    var my_group = "st_wc_merge_pending_add_01";	// this variable must match the above group name.
	...
	vscript_test_wc__addremove();
	this.gid_C_C = sg.wc.get_item_gid_path({"src":"dir_C"});
	...
	this.do_fsobj_mkdir("dir_C");
	vscript_test_wc__addremove();
	this.gid_WC_C = sg.wc.get_item_gid_path({"src":"dir_C"});

	var expect_test = new Array;
	expect_test["Added"] = [ "@/dir_C"
			       ];
        vscript_test_wc__confirm_wii(expect_test);
	...
	var expect_test = new Array;
	expect_test["Added (Merge)"] = [ this.gid_C_C ];
	expect_test["Added"]         = [ this.gid_WC_C ];
	// we don't get "Renamed" entries in a plain STATUS for
	// .isAdded or .isMergeCreated items because there is no
	// reference version.
	expect_test["Unresolved"]                    = [ this.gid_C_C, this.gid_WC_C ];
	expect_test["Choice Unresolved (Existence)"] = [ this.gid_C_C, this.gid_WC_C ];
        vscript_test_wc__confirm_wii(expect_test);
