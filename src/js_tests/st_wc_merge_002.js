...
function st_wc_merge_002()
{
    var my_group = "st_wc_merge_002";	// this variable must match the above group name.
	...
	// each changed entry was only changed in one of the
	// above csets (B or C), therefore we only exercise
	// either the code for:
	// [] "A-vs-L0 present, A-vs-L1 not present"
	// [] "A-vs-L0 not present, A-vs-L1 present"
	...
	vscript_test_wc__rename("dir_K", "W__dir_K");
	vscript_test_wc__rename("dir_L", "W__dir_L");
	vscript_test_wc__rename("dir_M", "W__dir_M");

	vscript_test_wc__rename("B__dir_A", "BW__dir_A"); // rename+rename
	vscript_test_wc__rename("C__dir_F", "CW__dir_F");

	vscript_test_wc__rename("B__dir_B", "dir_B"); // undo
	vscript_test_wc__rename("C__dir_G", "dir_G");

	var expect_test = new Array;
	expect_test["Renamed (WC!=A,WC==B,WC!=C)"] = [ "@/B__dir_C",
						       "@/B__dir_D",
						       "@/B__dir_E",
						     ];
	expect_test["Renamed (WC!=A,WC!=B,WC==C)"] = [ "@/C__dir_H",
						       "@/C__dir_I",
						       "@/C__dir_J",
						     ];
	expect_test["Renamed (WC!=A,WC!=B,WC!=C)"] = [ "@/W__dir_K",
						       "@/W__dir_L",
						       "@/W__dir_M",
						       "@/BW__dir_A",
						       "@/CW__dir_F",
	...
	expect_test["Renamed (WC==A,WC==B,WC!=C)"] = [ "@/dir_G"
						     ];
	vscript_test_wc__mstatus_confirm_wii(expect_test);
