...
function st_wc_merge_resolve_017()
{
    var my_group = "st_wc_merge_resolve_017";	// this variable must match the above group name.
	...
	// Do a CLEAN MERGE.
	//////////////////////////////////////////////////////////////////
	vscript_test_wc__print_section_divider("Checkout B, Create B1, and merge C into it.");
	...
	// TODO            Existence: @/file.txt

	// use C-based repo-path so we can avoid needing to use a wildcard in this command
	vscript_test_wc__resolve__accept("other", "@c/file.txt");
	...
	var expect_test = new Array;
	expect_test["Existence (A,!B,C,WC)"] = [ "@/file.txt" ];
	expect_test["Modified (C!=A,WC==C)"] = [ "@/file.txt" ];
	expect_test["Resolved"]              = [ "@/file.txt" ];
	expect_test["Choice Resolved (Existence)"] = [ "@/file.txt" ];
	vscript_test_wc__mstatus_confirm_wii(expect_test);
