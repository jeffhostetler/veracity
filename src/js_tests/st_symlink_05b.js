...
function st_symlink_05a()
{
    this.my_group = "st_symlink_05a";	// this variable must match the above group name.
	...
	//////////////////////////////////////////////////////////////////
	vscript_test_wc__print_section_divider("Try to rename file and symlink...");

	var result = sg.exec(vv, "rename", "sparse_1", "sparse_1_renamed");
	print(dumpObj(result, "sg.exec()", "", 0));
	testlib.ok( (result.exit_status == 0), "'vv rename sparse_1' should work" );

	var sparse_renamed_list = [ "@/sparse_1_renamed" ];

	if (bSymlinks)
	{
	    var result = sg.exec(vv, "rename", "sparse_link_1", "sparse_link_1_renamed");
	    print(dumpObj(result, "sg.exec()", "", 0));
	    testlib.ok( (result.exit_status == 0), "'vv rename sparse_link_1' should work" );

	    sparse_renamed_list.push( "@/sparse_link_1_renamed" );
	}

	// we should get a rename-vs-delete conflict when we update....

	//////////////////////////////////////////////////////////////////
	vscript_test_wc__print_section_divider("Try TEST update with sparse files...");

	var s = sg.wc.update({"rev" : cset_head,
			      "status": true,
			      "test" : true});
	print(dumpObj(s));

        var expect_test = new Array;
	expect_test["Added (Update)"]                = sparse_renamed_list;
	expect_test["Unresolved"]                    = sparse_renamed_list;
	expect_test["Choice Unresolved (Existence)"] = sparse_renamed_list;
        vscript_test_wc__confirm(expect_test, s.status);

	//////////////////////////////////////////////////////////////////
	vscript_test_wc__print_section_divider("Try to update...");
	...
	//////////////////////////////////////////////////////////////////
	vscript_test_wc__print_section_divider("Status after UPDATE....");

	var o = sg.exec(vv, "status", "--no-ignores", "--list-sparse", "--verbose");
	print(o.stdout);
