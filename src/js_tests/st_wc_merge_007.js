...
function st_wc_merge_007()
{
    var my_group = "st_wc_merge_007";	// this variable must match the above group name.
	...
	// clean file + wc dirt
	vscript_test_wc__write_file("file_0", (content_equal + content_W1));
	// extend changes made in B
	vscript_test_wc__write_file("file_1", ((content_B1 + content_equal) + content_W1));
	// undo changes made in B
	vscript_test_wc__write_file("file_2", (content_equal));
	// extend changes made in C
	vscript_test_wc__write_file("file_5", ((content_C1 + content_equal) + content_W1));
	// undo changes made in C
	vscript_test_wc__write_file("file_6", (content_equal));

	var expect_test = new Array;
	expect_test["Modified (WC!=A,WC!=B,WC!=C)"] = [ "@/file_0",
							"@/file_1",
							"@/file_5",
						      ];
	expect_test["Modified (WC==A,WC!=B,WC==C)"] = [ "@/file_2",
						      ];
	expect_test["Modified (WC==A,WC==B,WC!=C)"] = [ "@/file_6",
						      ];
	expect_test["Modified (WC!=A,WC==B,WC!=C)"] = [ "@/file_3",
							"@/file_4",
						      ];
	expect_test["Modified (WC!=A,WC!=B,WC==C)"] = [ "@/file_7",
							"@/file_8",
						      ];
	vscript_test_wc__mstatus_confirm_wii(expect_test);
