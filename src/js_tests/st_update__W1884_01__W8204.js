...
function st_update__W1884_01__W8204() 
{
    this.my_group = "st_update__W1884_01__W8204";	// this variable must match the above group name.
	...
	expect_test["Resolved"] = [ "@b/b1/a1", "@/b2/a2"  ];
	expect_test["Choice Resolved (Existence)"] = [ "@b/b1/a1", "@/b2/a2"  ];

	expect_test["Added (Update)"]  = [ "@/b3/a3",	// WD dirt in caused them to be kept, so they
					   "@/b4/a4" ];	// appear as added-by-update relative to X1.
	expect_test["Unresolved"]     = [ "@/b3/a3", "@/b4/a4" ];
	expect_test["Choice Unresolved (Existence)"] = [ "@/b3/a3", "@/b4/a4" ];
	vscript_test_wc__confirm_wii(expect_test);
