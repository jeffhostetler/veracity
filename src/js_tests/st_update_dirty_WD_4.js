...
function st_UpdateTestsWithDirtyWD() 
{
	...
	vscript_test_wc__print_section_divider("test_update_dirty_edits_with_revert_all");
        ...
        // modify some files that will be removed in the update-target.

        this.do_fsobj_append_file("file_1", "hello1");
	var buffer_file_11 = sg.file.read("dir_1/file_11");
	sg.file.write("dir_1/file_11", "dirt\n");

        var expect_test = new Array;
        expect_test["Modified"] = [ "@/file_1", "@/dir_1/file_11" ];
	...
	var name_file_11  = "@/dir_1/file_11";
        ...
        var expect_test = new Array;
        expect_test["Modified"]       = [ name_file_1, name_file_11 ];
        expect_test["Added (Update)"] = [ name_file_1, name_file_11 ];
        expect_test["Unresolved"]     = [ name_file_1, name_file_11 ];
        expect_test["Choice Unresolved (Existence)"] = [ name_file_1, name_file_11 ];
        vscript_test_wc__confirm(expect_test);

	// replace the content of file_11 with the original (pre-update, pre-dirt) content.

	var path_file_11 = name_file_11.substr(2);	// get rid of leading "@/" on path for write()
	sg.file.write(path_file_11, buffer_file_11);

        var expect_test = new Array;
        expect_test["Modified"]       = [ name_file_1               ];
        expect_test["Added (Update)"] = [ name_file_1, name_file_11 ];
        expect_test["Unresolved"]     = [ name_file_1, name_file_11 ];
        expect_test["Choice Unresolved (Existence)"] = [ name_file_1, name_file_11 ];
        vscript_test_wc__confirm(expect_test);
	...
	// and cleanly delete the (now) unmodified one.

	var backup_file_1  = vscript_test_wc__backup_name_of_file( name_file_1,  "sg00" );

	vscript_test_wc__revert_all__np( { "no_backups" : false }, "revert-all with backups." );
