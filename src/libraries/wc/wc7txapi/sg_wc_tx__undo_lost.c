						 ...
						 SG_wc_status_flags xu_mask)
{
	...
	SG_ERR_IGNORE(  SG_console(pCtx, SG_CS_STDERR,
							   "SG_wc_tx__undo_lost: source '%s' normalized to [domain %c] '%s'\n",
...
#if TRACE_WC_UNDO_DELETE
		SG_ERR_IGNORE(  SG_console(pCtx, SG_CS_STDERR,
								   "SG_wc_tx__undo_lost: destdir '%s' normalized to [domain %c] '%s'\n",
	...
	if (SG_WC_PRESCAN_FLAGS__IS_CONTROLLED_LOST(pLVI_Src->scan_flags_Live) == SG_FALSE)
	...
	SG_ERR_CHECK(  sg_wc_tx__rp__undo_lost__lvi_lvi_sz(pCtx, pWcTx,
													   pLVI_Src,
													   pLVI_DestDir,
													   pszEntryname_Dest,
													   pArgs,
