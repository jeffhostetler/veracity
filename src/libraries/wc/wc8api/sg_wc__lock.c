					   ...
					   const char * pszInput)
{
	...
	SG_ERR_IGNORE(  SG_console(pCtx, SG_CS_STDERR,
							   "SG_wc__lock: '%s' normalized to [domain %c] '%s'\n",
	...
	// TODO 2012/03/02 I'm guessing that they shouldn't be able to
	// TODO            lock something for which they have a queued
	// TODO            (pended) delete.  Right?
	// TODO
	// TODO            So does the delete code need to have a similar
	// TODO            restriction and/or auto-unlock it?
	if (statusFlags & SG_WC_STATUS_FLAGS__S__DELETED)
		SG_ERR_THROW2(  SG_ERR_VC_LOCK_CANNOT_LOCK_DELETED_ITEM,
						(pCtx, "%s", pszInput)  );
...
/**
 * Request a LOCK on one or more files.
 *
 * WARNING: This routine deviates from the model of most
 * WARNING: of the SG_wc__ level-8 and/or SG_wc_tx level-7
 * WARNING: API routines because we cannot just "queue" a
 * WARNING: lock like we do a RENAME with everything
 * WARNING: contained within the pWcTx; we actually have
 * WARNING: to update the locks dag and/or talk with the
 * WARNING: lock server.  So we only have a level-8 API
				 ...
				 const char * psz_repo_upstream)
{
	...
	SG_ASSERT_RELEASE_FAIL2(  (pszRepoDescriptorName && *pszRepoDescriptorName),
							  (pCtx, "SG_wc__lock: Could not get repo descriptor name.")  );
    ...
    // now we need the single parent csid of this working dir
    // if more than one, fail
	SG_ERR_CHECK(  SG_wc_tx__get_wc_baseline(pCtx, pWcTx, &psz_csid_parent, &bHasMerge)  );
	if (bHasMerge)
    ...
    // OK, we have all the pieces.  Time to request the lock

    SG_ERR_CHECK(  SG_vc_locks__request(
                pCtx,
                pszRepoDescriptorName,
                psz_repo_upstream,
                psz_username,
