...
/**
 * ADD <path> to version control in a stand-alone transaction.
 *
				...
				SG_varray ** ppvaJournal)
{
	SG_WC__WRAPPER_TEMPLATE__tj(
		...
		bTest,
		ppvaJournal,
		SG_ERR_CHECK(  SG_wc_tx__add(pCtx, pWcTx, pszInput, depth, bNoIgnores)  ));
}
							 ...
							 SG_varray ** ppvaJournal)
{
	SG_WC__WRAPPER_TEMPLATE__tj(
		...
		bTest,
		ppvaJournal,
		SG_ERR_CHECK(  SG_wc_tx__add__stringarray(pCtx, pWcTx, psaInputs, depth, bNoIgnores)  ));
}
