...
/**
 *
 * @file sg__wc__do_cmd_addremove.c
 *
 * @details 
 *
 */
						  ...
						  const SG_stringarray * psaArgs)
{
	...
	SG_ERR_CHECK(  SG_wc__addremove__stringarray(pCtx, NULL, psaArgs,
												 WC__GET_DEPTH(pOptSt),
												 WC__GET_NO_IGNORES(pOptSt),
												 pOptSt->bTest,
												 &pvaJournal)  );
	SG_ERR_IGNORE(  sg_report_addremove_journal(pCtx, pvaJournal, SG_TRUE, SG_TRUE, pOptSt->bVerbose)  );
